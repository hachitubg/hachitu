...
onMounted(async () => {
  const shared = await getSharedTournament()
  if (!shared) return
...
function handleAddMosconiParticipant(teamId: 'A' | 'B', name: string) {
  if (!currentTournament.value) return
  const result = addMosconiParticipant(currentTournament.value.id, teamId, name)
  if (!result.ok) alert(result.error)
}
...
function handleRenameMosconiTeam(teamId: 'A' | 'B', name: string) {
  if (!currentTournament.value) return
  const result = renameMosconiTeam(currentTournament.value.id, teamId, name)
  if (!result.ok) alert(result.error)
}
...
function handleConfigureMosconiMatch(matchId: string, lineupA: string[], lineupB: string[]) {
  if (!currentTournament.value) return
  const result = configureMosconiMatch(currentTournament.value.id, matchId, lineupA, lineupB)
  if (!result.ok) alert(result.error)
}
    ...
    <div class="mx-auto max-w-5xl px-6 py-8">
      ...
      <div class="animate-fade-up animate-delay-2">
        ...
        <TournamentDetail
          ...
          @add-mosconi-participant="handleAddMosconiParticipant"
          @add-participants-bulk="handleAddBulk"
          @remove-participant="handleRemoveParticipant"
          @edit-participant="handleEditParticipant"
          @rename-mosconi-team="handleRenameMosconiTeam"
          @start="handleStart"
          @configure-mosconi-match="handleConfigureMosconiMatch"
          ...
          :mosconi-score="currentTournament ? getMosconiScore(currentTournament) : { A: 0, B: 0 }"
        />
      </div>
