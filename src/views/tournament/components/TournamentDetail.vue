...
import MosconiCupView from './MosconiCupView.vue'
import MosconiTeamManager from './MosconiTeamManager.vue'
import ParticipantManager from './ParticipantManager.vue'
import RoundRobinView from './RoundRobinView.vue'

const props = defineProps<{
  tournament: Tournament
  mosconiScore: { A: number; B: number }
}>()

const emit = defineEmits<{
  back: []
  addParticipant: [name: string]
  addMosconiParticipant: [teamId: 'A' | 'B', name: string]
  addParticipantsBulk: [text: string]
  removeParticipant: [id: string]
  editParticipant: [id: string, name: string]
  renameMosconiTeam: [teamId: 'A' | 'B', name: string]
  start: [randomize: boolean]
  configureMosconiMatch: [matchId: string, lineupA: string[], lineupB: string[]]
...
const wbMatches = computed(() =>
  props.tournament.matches.filter((match) => match.bracket === 'winners'),
)
const lbMatches = computed(() =>
  props.tournament.matches.filter((match) => match.bracket === 'losers'),
)
const gfMatches = computed(() =>
  props.tournament.matches.filter((match) => match.bracket === 'grand-final'),
)
...
async function handleCopyShareLink() {
  ...
  shareMsg.value = isUrl
    ...
    : 'Dữ liệu quá lớn, đã sao chép mã giải đấu để bạn gửi thủ công.'
...
function getChampionName(): string {
  ...
  if (props.tournament.format === 'mosconi-cup') {
    if (props.mosconiScore.A === props.mosconiScore.B) return ''
    const winnerId = props.mosconiScore.A > props.mosconiScore.B ? 'A' : 'B'
    return props.tournament.mosconiTeams.find((team) => team.id === winnerId)?.name ?? ''
  }

  const matches = props.tournament.knockoutStage
    ? props.tournament.knockoutStage.matches
    : props.tournament.matches
  const nonByeMatches = matches.filter((match) => !match.isBye)
  if (nonByeMatches.length === 0 || props.tournament.format === 'round-robin') return ''
  ...
  return (
    props.tournament.participants.find((participant) => participant.id === finalMatch.winner)
      ?.name ?? ''
  )
}

function getMosconiTeamCount(teamId: 'A' | 'B'): number {
  return props.tournament.participants.filter((participant) => participant.teamId === teamId).length
}
</script>
...
<template>
  <div>
    ...
    <div v-if="isSetup" class="space-y-6">
      <MosconiTeamManager
        v-if="tournament.format === 'mosconi-cup'"
        :participants="tournament.participants"
        :teams="tournament.mosconiTeams"
        @add="(teamId, name) => emit('addMosconiParticipant', teamId, name)"
        @remove="(id) => emit('removeParticipant', id)"
        @edit="(id, name) => emit('editParticipant', id, name)"
        @rename-team="(teamId, name) => emit('renameMosconiTeam', teamId, name)"
      />
      ...
      >
        ...
        <div v-if="showSettings" class="space-y-4 border border-border-default bg-bg-surface p-4">
          ...
          <div v-if="tournament.format === 'mosconi-cup'" class="space-y-2">
            <p class="text-sm text-text-secondary">
              Đây là biến thể Mosconi theo luật riêng của bạn: 2 đội, mỗi đội 5 người, tổng cộng 20
              game cố định.
            </p>
            <p class="text-xs text-text-dim">
              Hệ thống sẽ tự chia ngẫu nhiên để mỗi người có đúng 3 trận đơn và 2 trận đôi.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="border border-border-default bg-bg-surface p-6">
        <div v-if="tournament.format !== 'mosconi-cup'" class="mb-4 flex items-center gap-3">
        ...
        <button
          ...
          :disabled="
            tournament.format === 'mosconi-cup'
              ? getMosconiTeamCount('A') !== 5 || getMosconiTeamCount('B') !== 5
        ...
        <p v-if="error" class="mt-2 text-xs text-accent-coral">{{ error }}</p>
        <p
          v-if="tournament.format !== 'mosconi-cup' && tournament.participants.length < 2"
        ...
        <p
          ...
          class="mt-2 text-xs text-text-dim"
        >
          Cần đúng 5 người cho mỗi đội để hệ thống chia đủ 20 game theo luật custom này.
        </p>
      </div>
    </div>
    ...
    <div v-if="isInProgress || isFinished" ref="tournamentContentRef">
      ...
      <MosconiCupView
        v-else-if="tournament.format === 'mosconi-cup'"
        :tournament="tournament"
        :editable="isInProgress"
        :score-a="mosconiScore.A"
        :score-b="mosconiScore.B"
        @submit-result="(id, a, b) => emit('submitResult', id, a, b)"
      />
    </div>
  </div>
