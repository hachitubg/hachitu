...
export function useGame() {
  type DiscussionStage = 'talking' | 'decision'
  ...
  const timeConfig = ref<TimeConfig>({
    ...
    discussionDecisionSeconds: 15,
    discussionExtendSeconds: 120,
    nightActionSeconds: 15,
    nominateSeconds: 30,
    explainSeconds: 60,
    hangVoteSeconds: 30,
    nightDelayMs: 3000,
  ...
  const nightActionTimeLeft = ref(0)
  ...
  const discussionStage = ref<DiscussionStage>('talking')
  const discussionTimeLeft = ref(0)
  const discussionDecisionTimeLeft = ref(0)
  const discussionSkipVotes = ref<Record<string, boolean>>({})
  const discussionDecisionVotes = ref<Record<string, 'extend' | 'sleep'>>({})
  let discussionTimer: ReturnType<typeof setInterval> | null = null
  let discussionDecisionTimer: ReturnType<typeof setInterval> | null = null
  ...
  let nightActionTimer: ReturnType<typeof setInterval> | null = null
  ...
  // ── Pause ──────────────────────────────────────────────────────────────────
  const discussionMajority = computed(() => Math.floor(livingPlayers.value.length / 2) + 1)

  const discussionSkipVoteCount = computed(
    () => Object.values(discussionSkipVotes.value).filter(Boolean).length,
  )

  const discussionExtendVoteCount = computed(
    () => Object.values(discussionDecisionVotes.value).filter((vote) => vote === 'extend').length,
  )

  const discussionSleepVoteCount = computed(
    () => Object.values(discussionDecisionVotes.value).filter((vote) => vote === 'sleep').length,
  )

  function clearDiscussionTalkTimer() {
    if (discussionTimer) {
  ...
  function clearDiscussionDecisionTimer() {
    if (discussionDecisionTimer) {
      clearInterval(discussionDecisionTimer)
      discussionDecisionTimer = null
    }
  }

  function clearDiscussionTimers() {
    clearDiscussionTalkTimer()
    clearDiscussionDecisionTimer()
  }

  function resetDiscussionVotes() {
    discussionSkipVotes.value = {}
    discussionDecisionVotes.value = {}
  }

  function startDiscussionTalkTimer() {
    clearDiscussionTalkTimer()
    discussionTimer = setInterval(() => {
      discussionTimeLeft.value--
      if (discussionTimeLeft.value <= 0) {
        clearDiscussionTalkTimer()
        void speak('Hết thời gian thảo luận.').then(() => startDiscussionDecision())
      }
    }, 1000)
  }

  function resolveDiscussionDecisionOnTimeout() {
    if (discussionExtendVoteCount.value > discussionSleepVoteCount.value) {
      extendDiscussion()
      return
    }

    goSleepAfterDiscussion()
  }

  function startDiscussionDecisionTimer() {
    clearDiscussionDecisionTimer()
    discussionDecisionTimer = setInterval(() => {
      discussionDecisionTimeLeft.value--
      if (discussionDecisionTimeLeft.value <= 0) {
        clearDiscussionDecisionTimer()
        resolveDiscussionDecisionOnTimeout()
      }
    }, 1000)
  }

  function startDiscussionDecision() {
    if (phase.value !== 'day-discussion') return

    clearDiscussionTalkTimer()
    discussionStage.value = 'decision'
    discussionDecisionVotes.value = {}
    discussionDecisionTimeLeft.value = timeConfig.value.discussionDecisionSeconds
    void speak('Kết thúc thảo luận. Mọi người chọn thêm thời gian hoặc đi ngủ.')
    startDiscussionDecisionTimer()
  }

  function extendDiscussion() {
    if (phase.value !== 'day-discussion') return

    clearDiscussionDecisionTimer()
    discussionStage.value = 'talking'
    discussionDecisionTimeLeft.value = 0
    resetDiscussionVotes()
    discussionTimeLeft.value =
      Math.max(discussionTimeLeft.value, 0) + timeConfig.value.discussionExtendSeconds
    void speak('Đã thêm thời gian thảo luận.')
    startDiscussionTalkTimer()
  }

  function goSleepAfterDiscussion() {
    if (phase.value !== 'day-discussion') return

    clearDiscussionTimers()
    discussionDecisionTimeLeft.value = 0
    resetDiscussionVotes()
    void speak('Mọi người đã quyết định đi ngủ.').then(() => startNextNight())
  }

  function clearNightActionTimer() {
    if (nightActionTimer) {
      clearInterval(nightActionTimer)
      nightActionTimer = null
    }
  }

  function getRandomAlivePlayerId(excludeIds: string[] = []) {
    const candidates = livingPlayers.value.filter((player) => !excludeIds.includes(player.id))
    if (candidates.length === 0) return null
    return shuffle(candidates)[0]?.id ?? null
  }

  function autoResolveNightStep(step: NightStep) {
    switch (step) {
      case 'disruptor': {
        const actorId = players.value.find(
          (player) => player.alive && player.role === 'disruptor',
        )?.id
        const targetId = getRandomAlivePlayerId(actorId ? [actorId] : [])
        if (targetId) setDisruptorTarget(targetId)
        confirmDisruptor()
        break
      }
      case 'wolves': {
        const wolves = livingPlayers.value.filter((player) =>
          ['wolf', 'wolf-cub', 'cursed-wolf'].includes(player.role),
        )
        const nonWolves = livingPlayers.value.filter((player) => player.faction !== 'wolf')
        for (const wolf of wolves) {
          if (wolfVotes.value[wolf.id]) continue
          const targetId = shuffle(nonWolves)[0]?.id
          if (targetId) setWolfVote(wolf.id, targetId)
        }
        confirmWolfVotes()
        break
      }
      case 'seer': {
        const actorId = players.value.find((player) => player.alive && player.role === 'seer')?.id
        const targetId = getRandomAlivePlayerId(actorId ? [actorId] : [])
        if (targetId) setSeerTarget(targetId)
        revealSeerResult()
        confirmSeer()
        break
      }
      case 'guard': {
        const targetId = getRandomAlivePlayerId(
          lastGuardTarget.value ? [lastGuardTarget.value] : [],
        )
        if (targetId) setGuardTarget(targetId)
        confirmGuard()
        break
      }
      case 'witch': {
        const choices: Array<'pass' | 'save' | 'kill'> = ['pass']
        if (!witchHealUsed.value && witchNightVictim.value) choices.push('save')
        if (!witchPoisonUsed.value && livingPlayers.value.length > 0) choices.push('kill')
        const choice = shuffle(choices)[0] ?? 'pass'
        if (choice === 'save') witchDoSave()
        if (choice === 'kill') {
          const targetId = getRandomAlivePlayerId()
          if (targetId) witchSetKill(targetId)
        }
        confirmWitch()
        break
      }
      case 'hunter': {
        const actorId = players.value.find((player) => player.alive && player.role === 'hunter')?.id
        const targetId = getRandomAlivePlayerId(actorId ? [actorId] : [])
        if (targetId) setHunterTarget(targetId)
        confirmHunter()
        break
      }
      case 'cupid': {
        const lovers = shuffle(livingPlayers.value).slice(0, 2)
        if (lovers[0]?.id && lovers[1]?.id) {
          setCupidTarget(1, lovers[0].id)
          setCupidTarget(2, lovers[1].id)
        }
        confirmCupid()
        break
      }
  ...
  function startNightActionTimer(step: NightStep) {
    clearNightActionTimer()
    nightActionTimeLeft.value = timeConfig.value.nightActionSeconds
    nightActionTimer = setInterval(() => {
      nightActionTimeLeft.value--
      if (nightActionTimeLeft.value <= 0) {
        clearNightActionTimer()
        autoResolveNightStep(step)
      }
    }, 1000)
  }
  ...
  function togglePause() {
    ...
    if (isPaused.value) {
      ...
      if (
        ...
        nightActionTimeLeft.value > 0
      ) {
        startNightActionTimer(nightStep.value)
  ...
  function skipRoleReveal() {
    if (phase.value !== 'role-reveal') return
    roleRevealShowing.value = false
    roleRevealIndex.value = players.value.length
    void speak('Tất cả đã xem xong. Game bắt đầu.').then(() => beginNight())
  }
  ...
  async function runNightStep(step: NightStep) {
    ...
    if (step === 'wolves') {
      hasLivingRoleHolder = players.value.some(
        (p) => p.alive && ['wolf', 'wolf-cub', 'cursed-wolf'].includes(p.role),
      )
    } else {
  ...
  function waitForNightAction(step: NightStep): Promise<void> {
    return new Promise((resolve) => {
      resolveNightAction = resolve
      startNightActionTimer(step)
    })
  }
  ...
  function revealSeerResult() {
    if (!seerTarget.value) return
  ...
  function startDiscussion() {
    ...
    void speak('Bắt đầu thảo luận.')
    startDiscussionTalkTimer()
  }

  function skipDiscussion() {
    startDiscussionDecision()
  }

  function castDiscussionSkipVote(playerId: string) {
    if (phase.value !== 'day-discussion' || discussionStage.value !== 'talking') return
    if (!livingPlayers.value.some((player) => player.id === playerId)) return
    if (discussionSkipVotes.value[playerId]) return
    ...
    if (discussionSkipVoteCount.value >= discussionMajority.value) {
      startDiscussionDecision()
    }
  }

  function castDiscussionDecisionVote(playerId: string, decision: 'extend' | 'sleep') {
    if (phase.value !== 'day-discussion' || discussionStage.value !== 'decision') return
    if (!livingPlayers.value.some((player) => player.id === playerId)) return
    ...
    if (discussionExtendVoteCount.value >= discussionMajority.value) {
      extendDiscussion()
      return
    }

    if (discussionSleepVoteCount.value >= discussionMajority.value) {
      goSleepAfterDiscussion()
    }
  }
  ...
  return {
    ...
    discussionStage,
    discussionTimeLeft,
    discussionDecisionTimeLeft,
    discussionSkipVotes,
    discussionDecisionVotes,
    discussionMajority,
    discussionSkipVoteCount,
    discussionExtendVoteCount,
    discussionSleepVoteCount,
    ...
    extendDiscussion,
    goSleepAfterDiscussion,
    castDiscussionSkipVote,
    castDiscussionDecisionVote,
    startNominate,
    castNomination,
    nextExplain,
    castHangVote,
