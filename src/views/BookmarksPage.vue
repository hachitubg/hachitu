...
const visibleBookmarkCategories = computed(() =>
  categories.filter((category) => (bookmarkCategoryCounts.value[category.id] ?? 0) > 0),
)
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 pt-20 sm:pt-28 pb-16 sm:pb-24">
      ...
      <div v-if="bookmarkedPages.length > 0">
        ...
        <CategoryFilter
          ...
          :available-categories="visibleBookmarkCategories"
          class="mt-12"
        />
      ...
      <div v-else class="mt-12 flex flex-col items-center justify-center py-16 text-center">
        ...
        <div class="mt-8 flex items-center gap-4">
          <RouterLink
            to="/"
            class="inline-flex items-center gap-2 border border-accent-coral bg-accent-coral/10 px-5 py-2.5 text-sm font-display text-accent-coral tracking-wide transition-all duration-300 hover:bg-accent-coral hover:text-bg-surface"
          >
            Khám phá ứng dụng
          </RouterLink>
          <button
            class="inline-flex items-center gap-2 border border-accent-sky bg-accent-sky/10 px-5 py-2.5 text-sm font-display text-accent-sky tracking-wide transition-all duration-300 hover:bg-accent-sky hover:text-bg-surface"
            @click="openFileDialog()"
          >
